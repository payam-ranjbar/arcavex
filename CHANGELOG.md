...
- **JPEG and WebP exporters** with quality/lossless control in `ExportOptions`. Lossless WebP is
  bit-exact (VP8L), not approximated. JPEG flattens transparency over opaque white deterministically.
...
- **Derived-asset LRU cache** (`$ARCAVEX_HOME/cache/derived/`): a large image drawn into a small
  ...
  byte budget. Disposable and never authoritative — a cold cache produces byte-identical output.
