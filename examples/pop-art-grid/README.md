...
A Warhol-style 2×2 grid: one source portrait repeated across four cells, each recoloured with a
different palette from the `pop-art` style pack and screened at a different halftone angle. It is
the acceptance artifact for Phase 3's effect system, shape/style packs, and shader work.
...
| Per-cell effect variation | `repeat` over cells drives each cell's halftone `angle` and duotone colours by expression |
| Effect presets | the halftone and grain effects are referenced as `{preset: ...}` |
| Colour fusion | per cell, `grade` + `duotone` (two matrices) fuse into one pass, then `posterize` (a table) |
...
luminance range so the halftone and duotone read well); it is not the reference poster.
