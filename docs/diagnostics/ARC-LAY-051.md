...
**Typical fix:** Lower min_size so a smaller, fitting size exists, enlarge the box, or switch the policy to 'truncate'. Note the direction: 'min_size' is the floor of the shrink search, so raising it removes the only candidates that could still fit and makes the overflow strictly worse.
